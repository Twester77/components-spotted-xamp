 ...
 *      todo post de perdidos. O token vai via POST (multipart) — o
 *      fetch já usa new FormData(form), que pega o hidden automaticamente.
 *    - ADICIONADO htmlspecialchars() em 3 pontos de exibição de dados
 *      que vinham do banco sem escape (defesa em profundidade):
 *        1. $_SESSION['usuario_nome'] no painel de sessão
 *        2. $linha['username'] no <small>@ do card
 *        3. $linha['username'] no aria-label do link "Ver detalhes"
 *      Embora username tenha validação na entrada (regex), a
 *      exibição sempre deve ser escapada — regra da Djê.
 */
...
<main class="main-perdidos" id="conteudo-principal">
    ...
    <?php else: ?>
        <div class="painel-sessao" role="region" aria-label="Informações da Sessão">
            <p>Logado como: <strong><?php echo htmlspecialchars($_SESSION['usuario_nome'] ?? '', ENT_QUOTES, 'UTF-8'); ?></strong> <span aria-hidden="true">🎓</span></p>
                    ...
                    <article class="spotted-card perdidos-item <?php echo ($linha['subcategoria'] == 'achei') ? 'card-achado' : 'card-perdido'; ?>">
                        <div class="card-header">
                            <span class="category-tag">
                                ...
                                <?php endif; ?>
                                <small>@<?php echo !empty($linha['username']) ? htmlspecialchars($linha['username'], ENT_QUOTES, 'UTF-8') : "Anônimo"; ?></small>
                            </span>
                        ...
                        <div class="card-footer">
                            <a href="comentarios-post.php?id=<?php echo (int)$linha['id']; ?>" class="link-fofoca" aria-label="Ver detalhes / ajudar a encontrar o objeto de @<?php echo !empty($linha['username']) ? htmlspecialchars($linha['username'], ENT_QUOTES, 'UTF-8') : 'Anônimo'; ?>">
