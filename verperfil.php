...
<style>
    ...
    .avatar-main {
        border-color:
            <?php echo $is_presenca ? 'var(--dourado)' : '#fff'; ?>
            !important;
    }
            ...
            <?php if (!empty($dados['atletica_id'])): ?>
                <a href="atleticas.php?id=<?php echo urlencode($dados['atletica_id']); ?>">
                    <img src="badges/<?php echo htmlspecialchars($dados['atletica_id']); ?>.png"
                        class="insignia-atletica-bottom" title="Ver comunidade da Atlética">
                </a>
            <?php endif; ?>
        </div>
...
<script>
    let offset = 0;
    // Pega o nome do usuário direto da URL (ex: ?user=menecas)
    const urlParams = new URLSearchParams(window.location.search);
    const usuarioAlvo = urlParams.get('user'); 

    function carregarFeedPerfil() {
        // AQUI ESTÁ O SEGREDO: tipo=perfil e o user da URL
    ...
    carregarFeedPerfil(); // Chama a primeira vez
</script>
