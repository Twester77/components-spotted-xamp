/* A FENDA - ENGINE CENTRAL (V5 - Blindagem Total) */
/* Resolve "ghost swipe" e salto no clique com lock de estado e reset atômico */

// 1. FUNÇÕES GLOBAIS DE CONTROLE (inalteradas)
...
// ================================================================
// FÍSICA BLINDADA (V5) – SEM MULETAS GEOMÉTRICAS, COM LOCK GLOBAL
// ================================================================
window.iniciarFisicaSwipe = function () {
    ...
    // 🔒 BARREIRA DE ESTADO: se o card já está pronto, aborta
    if (cardTopo.getAttribute('data-swipe-pronto') === 'true') {
        console.log("[SWIPE] Card", cardTopo.dataset.id, "já ativo. Abortando re-inicialização.");
        return;
    }

    // 🧹 Limpeza de qualquer listener residual (segurança)
    if (cardTopo._swipeCleanup) {
        cardTopo._swipeCleanup();
        delete cardTopo._swipeCleanup;
    }

    // 🧱 RESET ATÔMICO: garante que o card está exatamente no centro, sem transição
    ...
    // Função de opacidade elástica (ease-in-out cúbico)
    ...
    const onMove = (e) => {
        ...
        rafId = requestAnimationFrame(() => {
            ...
            if (currentX > 40 && feedbackDir) {
                ...
                // reseta feedbacks se não houver movimento forte
    ...
    const onStart = (e) => {
        // Bloqueia se clicar em elementos interativos dentro do card
        ...
        // 🛡️ IMPEDE O "SALTO" – verifica se o card está na posição zero. Se não estiver, força reset sem transição.
        const currentTransform = cardTopo.style.transform;
        if (currentTransform !== 'translate(-50%, -50%)' && currentTransform !== '') {
            console.warn("[SWIPE] Card fora da posição inicial. Resetando atômicamente.");
            cardTopo.style.transition = 'none';
            cardTopo.style.transform = 'translate(-50%, -50%)';
            // pequeno delay para o navegador aplicar o reset
            setTimeout(() => {}, 5);
        }
        ...
        // Listeners globais (garante que o movimento continue mesmo fora do card)
    ...
    console.log("[SWIPE] Card", cardTopo.dataset.id, "blindado com sucesso.");
};

// ================================================================
// RECICLAGEM SEGURA DO CARD
// ================================================================
...
// ================================================================
// CARREGAMENTO INFINITO COM LOCK DE ESTADO (SEM RACE CONDITION)
// ================================================================
...
// ================================================================
// LOAD FINAL: CONFIGURAÇÃO DO ABASTECIMENTO DE PILHA (INFINITE SCROLL)
// ================================================================
window.addEventListener('load', () => {
    ...
    if (document.body.classList.contains('modo-swipe-ativo')) {
        ...
        // Limpeza nuclear: remove qualquer listener fantasma
    ...
    // ============================================================
    // ABASTECIMENTO DE PILHA COM LOCK TOTAL (sem race condition)
    // ============================================================
    let offsetSwipe = 10;
    ...
    window.abastecerPilhaFenda = function () {
        // 🔒 LOCK: se já estiver carregando, agenda uma nova tentativa
        if (carregandoSwipe) {
            if (!fetchAgendado) {
            ...
            fetch(`motor-feed.php?offset=${offsetSwipe}&categoria=${categoria}&tipo=geral`)
                ...
                .then(html => {
                    if (html.trim() !== "FIM_DADOS") {
                        ...
                        // 🧱 REFLOW FORÇADO E ATIVAÇÃO SEGURA
                        setTimeout(() => {
                            const containerCheck = document.querySelector('.container-feed');
                            if (containerCheck) containerCheck.offsetHeight; // força reflow
                            window.iniciarFisicaSwipe();
                            console.log("[FETCH] Novos cards inseridos e físico reiniciado.");
                        }, 50);
                    } else {
