...
// ===============================================================
// FÍSICA NATIVA SUPREMA (MANOPLA DO INFINITO - VERSÃO GEOMÉTRICA)
// ===============================================================
window.iniciarFisicaSwipe = function () {
    ...
    // 🛡️ BARREIRA CÓSMICA: Se o card já tem física ativa, aborta para não duplicar!
    ...
    // Função auxiliar para opacidade elástica (ease-in-out cúbico)
    ...
    const onMove = (e) => {
        ...
        rafId = requestAnimationFrame(() => {
            ...
            // FEEDBACK VISUAL ELÁSTICO (curva ease-in-out)
    ...
    const onEnd = (e) => {
        ...
        // 🔓 DESCONGELA O CONTAINER (restaura altura e overflow)
        const containerFeed = document.querySelector('.container-feed');
        if (containerFeed) {
            containerFeed.style.height = '';
            containerFeed.style.overflow = '';
        }
    ...
    const onStart = (e) => {
        ...
        // 🛡️ CONGELA O CONTAINER PARA EVITAR REFLOW DURANTE O ARRASTE
        const containerFeed = document.querySelector('.container-feed');
        if (containerFeed) {
            containerFeed.style.height = containerFeed.offsetHeight + 'px';
            containerFeed.style.overflow = 'hidden';
        }
...
window.addEventListener('load', () => {
    ...
    // ============================================================
    // LOCK DE FETCH COM FILA + REFLOW FORÇADO
    // ============================================================
    let offsetSwipe = 10;
    ...
    window.abastecerPilhaFenda = function () {
        // Se já está carregando, agenda nova tentativa para depois
        if (carregandoSwipe) {
            if (!fetchAgendado) {
            ...
            fetch(`motor-feed.php?offset=${offsetSwipe}&categoria=${categoria}&tipo=geral`)
                ...
                .then(html => {
                    if (html.trim() !== "FIM_DADOS") {
                        ...
                        // 🚀 O ESTALO DE DEDOS: Dá 50ms + REFLOW FORÇADO
                        setTimeout(() => {
                            // Força o navegador a calcular o layout antes de ativar a física
                            const containerCheck = document.querySelector('.container-feed');
                            if (containerCheck) containerCheck.offsetHeight; // reflow forçado
