...
*/
include_once 'conexao.php'; // Puxa a conexão e a função de menções[cite: 9]
session_start();
...
// 2. CONSTRUÇÃO DA QUERY BASE[cite: 7, 11]
...
// Filtro por Categoria (Se selecionado nos botões da Fenda)
if (!empty($categoria)) {
...
if ($tipo_feed === 'perfil' && !empty($user_alvo)) {
    // Filtra posts de um usuário específico (@username)
    $filtros[] = "u.username = '$user_alvo'";
} elseif ($tipo_feed === 'pessoal' && isset($_SESSION['usuario_id'])) {
    // Filtra apenas os MEUS posts[cite: 6]
...
// 3. RENDERIZAÇÃO DO HTML (Igual ao seu padrão de cards)[cite: 7, 11]
if (mysqli_num_rows($resultado) > 0) {
    while ($linha = mysqli_fetch_assoc($resultado)) {
        ...
        <article id="post-<?php echo $post_id_atual; ?>" class="spotted-card <?php echo $categoria_atual; ?>" style="position: relative;">
            ...
            <!-- Menu de Opções (Excluir/Denunciar)[cite: 7] -->
